...
test('empty recommendations keep the section visible with an empty state',
  async()=>{
    const container={
      children:['skeleton'],
    ...
    const section={
      hidden:true,
      dataset:{
        recommendationEndpoint:'/api/recommendation/personalized',
        contextualRerank:'false'
      },
      querySelector(selector){
        if(selector==='[data-recommendation-list]'){
          return container;
        }
        if(selector==='[data-recommendation-status]'){
          return status;
        }
        if(selector==='[data-recommendation-status-message]'){
    ...
    const recommendations=await engine.enhanceSection(section);

    assert.deepEqual(recommendations,[]);
    assert.equal(section.hidden,false);
    assert.equal(section.dataset.recommendationState,'empty');
    assert.equal(status.hidden,false);
    assert.match(statusMessage.textContent,/Chưa có gợi ý phù hợp/);
