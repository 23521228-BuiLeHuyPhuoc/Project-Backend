...
test('ignore feedback contributes to the retraining threshold',async()=>{
  ...
  assert.equal(await scheduler.getInteractionCount(),3);
  assert.equal(
    capturedQuery.type.$in.includes('recommendation_ignore'),
    true
  );
});
