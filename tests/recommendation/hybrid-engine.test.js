...
const createHybridFixture=({
  ...
  const models={
    ...
    UserInteraction:{
      ...
      find:()=>createQuery(liveInteractions)
    }
  };
...
test('hybrid ranking favors collaborative score for established users',
  async()=>{
    ...
    assert.equal(
      first[0].weights.strategy,
      'established_without_preferences'
    );
...
test('click feedback remains a positive signal instead of excluding the tour',
  async()=>{
    const {engine}=createHybridFixture({
      liveInteractions:[{
        tourId:'tour-a',
    ...
    assert.equal(
      recommendations.some(item=>item.tourId==='tour-a'),
      true
    );
  });

test('completed and rated tours are excluded from hybrid recommendations',
  async()=>{
    const {engine}=createHybridFixture({
      liveInteractions:[
        {tourId:'tour-a',type:'purchase'},
        {tourId:'tour-b',type:'rating'}
      ]
    });

    const recommendations=await engine.getRecommendations('user-1');

    assert.equal(recommendations.some(item=>item.tourId==='tour-a'),false);
    assert.equal(recommendations.some(item=>item.tourId==='tour-b'),false);
  });

test('explicit exclusions remove the current detail tour only',async()=>{
  const {engine}=createHybridFixture();

  const recommendations=await engine.getRecommendations('user-1',{
    excludeTourIds:['tour-b']
  });
