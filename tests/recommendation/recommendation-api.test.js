...
const startApi=async options=>{
  ...
  const engine={
    ...
    async getRecommendations(userId,requestOptions){
      engine.lastUserId=userId;
      engine.lastRecommendationOptions={...requestOptions};
      engine.personalizedCalls=(engine.personalizedCalls || 0)+1;
      return recommendations.slice(0,requestOptions.limit);
    },
...
test('personalized API forwards the current detail tour exclusion',async t=>{
  const api=await startApi();
  t.after(api.close);

  const response=await fetch(
    `${api.baseUrl}/personalized?limit=2&excludeTourId=${ids.source}`,
    {headers:{'x-test-user':'authenticated'}}
  );

  assert.equal(response.status,200);
  assert.deepEqual(api.engine.lastRecommendationOptions.excludeTourIds,[
    ids.source
  ]);
});
...
test('personalized API caches calculations and keeps request IDs unique',
  async t=>{
    ...
    assert.equal(fourthResponse.headers.get('x-recommendation-cache'),'HIT');
    await fetch(`${api.baseUrl}/feedback`,{
      method:'POST',
      headers:{
        'content-type':'application/json',
      ...
      body:JSON.stringify({
        eventId:'feedback_ignore_invalidation_1234',
        requestId:first.data.requestId,
        tourId:ids.second,
        action:'ignore',
        position:1,
        source:'home',
        pagePath:'/',
        scores:{final:0.6,content:0.6,collaborative:0.6,popularity:0.6}
      })
    });
    const fifthResponse=await request();

    assert.equal(fifthResponse.headers.get('x-recommendation-cache'),'MISS');
    assert.equal(api.engine.personalizedCalls,3);
  });
